...
//FUNCTIONS
void master_file_reader(char *master_file, int con_mod);
void load_dictionary(char file[string_length]);

int isMultipleof5(int n);
void Modify_reactivation_elders(int year, int parameters, double GP[parameters][AG][EG]);
...
void master_file_reader(char *master_file, int con_mod){	
    ...
    if (strcmp(name_country, simulation_name) != 0)
        sprintf(path_fitted_inputs, "../Outputs/%s_cm_%d/Iter/%s/Fitted_inputs", name_country, con_mod, simulation_name);
    else
        sprintf(path_fitted_inputs, "../Outputs/%s_cm_%d/Fitted_inputs", simulation_name, con_mod);	
...
void Modify_reactivation_elders(int year, int parameters, double GP[parameters][AG][EG]){
	...
	if(isMultipleof5(year)==0){
        //i = 9 -> rn reactivation rate
	    i = 9;
	    for (a=AG-1; a>0; a--){
		    for(e=0; e<EG; e++){
			    GP[i][a][e] = GP[i][a-1][e];			
	    ...
	    /*fprintf(out, "%d ", 2000+year);
	    for(j=0; j<AG; j++){
	        fprintf(out, "%.05lf ", GP[14][j][0]);
	    }
	    fprintf(out, "\n");*/
	}
	//fclose(out);
...
void load_vaccine_modifiers_H(char file[string_length], int parameters, double varp[parameters][AG][EG])
{
    ...
    scan=fscanf(fich,"waning_harris = %lf \n",&trash);
    scan=fscanf(fich, "transition age S_L_R efficacy e_low e_hi coverage c_low c_hi waning w_low w_hi blocking b_low b_hi\n");
    while(fscanf(fich, "%s %d %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", tt, &age, slr, &efficacy, &eff_low, &eff_hi, &coverage, &cov_low, &cov_hi, &wan, &waning_low, &waning_hi, &block, &blocking_low, &blocking_hi)==15){
        max_num++;
    }
