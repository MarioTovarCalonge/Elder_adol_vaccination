...
for(state in states){
    ...
    #Elder
    df_st_M1_w10e <- subset(data_plotM1e, waning==10 & state==target_pop)
    df_st_M1_w10e$cmod <- as.factor(rep('Pairwise correction', length(Vac_text)))
    
    df_st_M3_w10e <- subset(data_plotM3e, waning==10 & state==target_pop)
...
pdf(paste0('CompW10', country, '.pdf'), width = 17, height = 10)
