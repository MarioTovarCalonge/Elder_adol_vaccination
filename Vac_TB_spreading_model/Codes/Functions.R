...
#' @examples
#' f("China", 0)
set_TBsim_workspace <- function(ctr, itr, mod_con){
    ...
    pathToMF <- create_maste_file(ctr, itr, available[ctr], mod_con)
     
    if(itr==0){ 
        output_dir <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/")
        if (!dir.exists(output_dir)){
            ...
            create_initial_fit_param(ctr, 0, mod_con)
            system(paste0("cp ../Inputs/General_Inputs/Initial_Conditions/initial_conditions_Base.txt ../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Fitted_inputs/Initial_Conditions/initial_conditions.txt"))
        }
    }
        
    if(itr>0){
        output_dir <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Initial_Conditions/")
        if (!dir.exists(output_dir)){
            dir.create(output_dir, recursive = TRUE)
            destfile <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Fitted_inputs/Parameters_to_fit.txt")
            if(!file.exists(destfile)){
                create_initial_fit_param(ctr, itr, mod_con)
            } else{
                system(paste0("cp ../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Fitted_inputs/Parameters_to_fit.txt ../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Parameters_to_fit.txt"))
            }
            system(paste0("cp ../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Fitted_inputs/Initial_Conditions/initial_conditions.txt ../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Initial_Conditions/initial_conditions.txt"))
        }
    }
    return(pathToMF)
}
...
#' @keywords masterfile
create_maste_file <- function(ctr, itr, reg, mod_con){
    path_MF <- tempdir()
    print(path_MF)
    ...
    if(itr==0){
        ...
        if(reg==0){ aux <- sprintf("../Inputs/Contacts/Polymod_contacts") }
        if(reg==1){ aux <- sprintf("../Inputs/Contacts/Asia_contacts") }
        if(reg==2){ aux <- sprintf("../Inputs/Contacts/Africa_contacts") }
        
        if(mod_con==0 || mod_con==1){ aux2 <- paste0(aux, ".txt")
        }
        if(mod_con==2 || mod_con==3){ aux2 <- paste0(aux, "_pi.txt")
        }
	    write(sprintf("contacts %s \n", aux2), temp_path, append=T);
        ...
        path_global_par <- paste0("../Inputs_Iter/", ctr, "_cm_", sprintf("%d", mod_con), "/Global/global_parameters_mc_update_", sprintf("%03d", itr),".txt")
	    write(paste0("global_parameters ", path_global_par), temp_path, append=T)

        path_reg_par <- paste0("../Inputs_Iter/", ctr, "_cm_", sprintf("%d", mod_con), "/Regional/regional_parameters_literature_", sprintf("%03d", itr),".txt")
	    write(paste0("regional_parameters ", path_reg_par), temp_path, append=T)
	    
        if(reg==0){ aux <- paste0("../Inputs_Iter/", ctr, "_cm_", sprintf("%d", mod_con), "/Contacts/Polymod_contacts_") }
        if(reg==1){ aux <- paste0("../Inputs_Iter/", ctr, "_cm_", sprintf("%d", mod_con), "/Contacts/Asia_contacts_") }
        if(reg==2){ aux <- paste0("../Inputs_Iter/", ctr, "_cm_", sprintf("%d", mod_con), "/Contacts/Africa_contacts_") }
        if(mod_con==0 || mod_con==1){ aux2 <- paste0(aux, sprintf("%03d", itr), ".txt") }
        if(mod_con==2 || mod_con==3){ aux2 <- paste0(aux, "pi_", sprintf("%03d", itr), ".txt" ) }
	    write(sprintf("contacts %s \n", aux2), temp_path, append=T);

        path_demo <- paste0("../Inputs_Iter/", ctr, "_cm_", sprintf("%d", mod_con), "/Demo/Demographic_pyramid_", sprintf("%03d", itr),".txt")
	    write(paste0("demography ", path_demo), temp_path, append=T)
...
#' @keywords Parameters
create_initial_fit_param <- function(ctr, itr, mod_con){
    param <- c("d0", "beta0", "beta1", "d1", "dist_est")
    param_up <- c(12, 50, 20, 0.2, 0.5)
    param_low <- c(0, 0, -0.2, -0.2, -0.5)
    if(itr==0){
        param_file <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Fitted_inputs/Parameters_to_fit.txt")
    } else{
        param_file <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Parameters_to_fit.txt")
    }
...
#' @keywords Error
create_recal_parameters <- function(country, ini, fin, mod_con, owrite=TRUE){
    ...
    if(.Platform$OS.type == "unix") {
        system(paste0("mkdir -p ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Demo/"))
        system(paste0("mkdir -p ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Burden/"))
        system(paste0("mkdir -p ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Global/"))
        system(paste0("mkdir -p ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Regional/"))
    } else {
        system(paste0("mkdir ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Demo/"))
        system(paste0("mkdir ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Burden/"))
        system(paste0("mkdir ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Global/"))
        system(paste0("mkdir ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Regional/"))
    }
    ...
    for(a in ini:fin){
        file_out = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Demo/Demographic_pyramid_', sprintf("%03d", a), '.txt')
    ...
    for(a in ini:fin){
        ...
        file_out = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Burden/TB_burden_', sprintf("%03d", a), '.txt')
    ...
    for(a in ini:fin){
        file_out = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Global/global_parameters_mc_update_', sprintf("%03d", a), '.txt')
    ...
    for(a in ini:fin){
        file_out = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Regional/regional_parameters_literature_', sprintf("%03d", a), '.txt')
...
#' @return This function does return nothing.
#' @keywords Contacts
create_recal_contacts <- function(country, ini, fin=ini, reg, mod_con, owrite=TRUE){
    if(reg==1){
        doc <- '../Inputs/Contacts/Asia_contacts'
    } else if(reg==2){
        doc <- '../Inputs/Contacts/Africa_contacts'
    } else if(reg==0){
        doc <- '../Inputs/Contacts/Polymod_contacts'
    } else if(reg==3){
        doc <- '../Inputs/Contacts/LATAM_contacts'
    }  
    if(mod_con==0 || mod_con==1){ doc <- paste0(doc, ".txt") }
    if(mod_con==2 || mod_con==3){ doc <- paste0(doc, "_pi.txt" ) }
    ...
    if(.Platform$OS.type == "unix") {
        system(paste0("mkdir -p ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Contacts/"))
    } else {
        system(paste0("mkdir ../Inputs_Iter/", country, "_cm_", sprintf("%d", mod_con), "/Contacts/"))
    }
    ...
    for(a in ini:fin){
        ...
        if(reg==1){
            output = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Contacts/Asia_contacts_')
        } else if(reg==2){
            output = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Contacts/Africa_contacts_')
        } else if(reg==0){
            output = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Contacts/Polymod_contacts_')
        } else if(reg==3){
            output = paste0('../Inputs_Iter/', country, '_cm_', sprintf("%d", mod_con), '/Contacts/LATAM_contacts_')
        }
        if(mod_con==0 || mod_con==1){ output <- paste0(output, sprintf("%03d", a), ".txt") }
        if(mod_con==2 || mod_con==3){ output <- paste0(output, "pi_", sprintf("%03d", a), ".txt" ) }
        if (owrite){
...
demography_fit <- function(demo_file, ctr, itr, y_min_fit=2000, y_max_fit=2018, y_min_run=2000, y_max_run=2100, con_mod){
    options(scipen=999)
    if(itr==0){
        fitting_polynomial_file = paste0("../Outputs/", ctr, "_cm_", sprintf("%d", con_mod), "/Fitted_inputs/Pyr_coef_fit.txt")
        running_polynomial_file = paste0("../Outputs/", ctr, "_cm_", sprintf("%d", con_mod), "/Fitted_inputs/Pyr_coef_run.txt")
    } else{
        fitting_polynomial_file = paste0("../Outputs/", ctr, "_cm_", sprintf("%d", con_mod), "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Pyr_coef_fit.txt")
        running_polynomial_file = paste0("../Outputs/", ctr, "_cm_", sprintf("%d", con_mod), "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Pyr_coef_run.txt")
    }
...
#' @param itr An integer indicating the number of the error calculation iteration.
#' @return This function does not return anything.
rewritte_parameters_to_fit <- function(x, ctr, itr=0, mod_con){
    if(itr==0){
        par_dest <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), "/Fitted_inputs/Parameters_to_fit.txt")
    } else{
        par_dest <- paste0("../Outputs/", ctr, "_cm_", sprintf("%d", mod_con), '/Iter/', ctr, '_', sprintf("%03d", itr), "/Fitted_inputs/Parameters_to_fit.txt")
    }
...
fitter <- function(ctr, itr, fit_span=19, mod_cts=0, MFILE, func_fitness = fitness_INC_MORT, ...) {
    ...
    auxPath <- path.expand("~")
    if(itr==0){
        pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctr, sprintf("_cm_%d", mod_cts), '/Object_Folder/Baseline')
    } else{
        pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctr, sprintf("_cm_%d", mod_cts), '/Object_Folder/Iter_', sprintf("%03d", itr))
    }
...
fitter_nlfb <- function(x, ctr, itr, fit_span=19, mod_cts=3, MFILE, func_fitness = fitness_INC_MORT, ...) {
    rewritte_parameters_to_fit(x, ctr, itr, mod_cts)
    temp = numeric(3*1531*fit_span)
...
fitness_INC_MORT <- function(out, ...) {
    ...
    path_window <- paste0("../Inputs/General_Inputs/window_2100.txt")
    wind_info <- read.table(path_window, header=F)
    fit_start <- wind_info$V1 - 1999
    ...
    fitness1 <- numeric(ages_fit)
    fitness2 <- numeric(ages_fit)
    
    for(j in fit_start:ages_fit){
        i = j - fit_start + 1
        if((inc_new[i] - Burden$Inc[j]) < 0.0){
            fitness1[j] = (inc_new[i] - Burden$Inc[j])/(-mean(Burden$Inc))
        } else{
            fitness1[j] = (inc_new[i] - Burden$Inc[j])/(mean(Burden$Inc))
        }
    }
    ...
    for(j in fit_start:ages_fit){
        i = j - fit_start + 1
        if((mort_new[i] - Burden$Mort[j]) < 0.0){
            fitness2[j] = (mort_new[i] - Burden$Mort[j])/(-mean(Burden$Mort))
        } else{
            fitness2[j] = (mort_new[i] - Burden$Mort[j])/(mean(Burden$Mort))
        }
    }
...
#' A Function used for plotting global incidende and mortality trends if and only if ggplot2 package is included.
#'
...
#' @param running A logical parameter indicating whether plotting the result of running step is desired. Default is T and changes to F if running is set to F
#' @param FuncFitness A function for calculate fitness. It can be user-defined and should return a vector of residuals after evaluation of fitness criterion. Default is set to WHO data fitness.  
#' @param ... Arguments needed by FuncFitness. Default is none.
...
simTB_iter <- function(ctry, iteration=0, mode_contact=3, fitting=T, running=T, running_vaccine=F, FuncFitness = fitness_INC_MORT, ...){
    if(iteration==0){
        ...
        fitting_param_file = paste0("../Outputs/", ctry, "_cm_", sprintf("%d", mode_contact), "/Fitted_inputs/Parameters_to_fit.txt")
    } else{
        demography_file = paste0("../Inputs_Iter/", ctry, "_cm_", sprintf("%d", mode_contact), "/Demo/Demographic_pyramid_", sprintf("%03d", iteration), ".txt")
        
        TB_file = paste0("../Inputs_Iter/", ctry, "_cm_", sprintf("%d", mode_contact), "/Burden/TB_burden_", sprintf("%03d", iteration), ".txt")
        
        fitting_param_file = paste0("../Outputs/", ctry, "_cm_", sprintf("%d", mode_contact), "/Iter/", ctry, sprintf("_%03d", iteration), "/Fitted_inputs/Parameters_to_fit.txt")
    }
    
    master_file <- set_TBsim_workspace(ctry, iteration, mode_contact)
    ...
    demography_fit(demography_file, ctry, iteration, year_min, year_max, year_ini_demo, year_fin_demo, mode_contact)
    ...
    if(fitting){
        ...
        if(require(minpack.lm)){
            ...
            auxPath <- path.expand("~")
            if(iteration==0){
                weightPathOut <- paste0(auxPath, "/TB_SIM_OUT/", ctry, sprintf("_cm_%d", mode_contact), '/Object_Folder/Baseline/poids_', ctry, ".txt")
            } else{
                weightPathOut <- paste0(auxPath, "/TB_SIM_OUT/", ctry, sprintf("_cm_%d", mode_contact), '/Object_Folder/Iter_', sprintf("%03d", iteration), '/poids_', ctry, ".txt")
            }

                
            modfit <- nls.lm(par = guess, fn = fitter_nlfb, lower = lb, upper = ub, control=c(watch=F, smallsstest=F, rofftest=F, femax=10000), ctr = ctry, itr = iteration, fit_span=span, mod_cts=mode_contact, MFILE=master_file, func_fitness = FuncFitness, ...) #Residual function based
            x <- coef(modfit)
            rewritte_parameters_to_fit(x, ctry, iteration, mode_contact)
            ...
            if(fitness > 2){
                ...
                modfit <- nlfb(start = guess, resfn = fitter_nlfb, lower = lb, upper = ub, trace = FALSE, control=c(watch=F, smallsstest=F, rofftest=F, femax=10000), ctr = ctry, itr = iteration, fit_span=span, mod_cts=mode_contact, MFILE=master_file, func_fitness = FuncFitness, ...)
                x <- coef(modfit)
                rewritte_parameters_to_fit(x, ctry, iteration, mode_contact)
            ...
            wout2 <- read.table(weightPathOut, header=F)
    ...
    if(running){
        ...
        temp_C <- .C("R2C_runner", mf = master_file, as.integer(iteration), as.integer(mode_contact), vaccinate=as.integer(log_vac), objeto1 = temp1, objeto2 = temp2, out_handler = as.integer(error_handler))
        ...
        auxPath <- path.expand("~")
        if(iteration==0){
            pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctry, sprintf("_cm_%d", mode_contact), '/Object_Folder/Baseline')
        } else{
            pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctry, sprintf("_cm_%d", mode_contact), '/Object_Folder/Iter_', sprintf("%03d", iteration))
        }
        if (!dir.exists(pathout)){
            dir.create(pathout, recursive = TRUE)
        }
        ...
        if(save_vaccine){
            ...
            aux_info <- unlist(info[4])
            aux_Harris_waning <- unlist(info[3])
            #print(target_type)
            ...
            if(iteration==0){
                pathout_vaccine <- paste0(auxPath, "/TB_SIM_OUT/", ctry, sprintf("_cm_%d", mode_contact), "/Vaccines/", line_tran, "/",
                                sprintf("T_%d-E_%.2f-C_%.2f-W_%.2f", as.integer(aux_info["age"]), aux_info["efficacy"], aux_info["coverage"], aux_Harris_waning[1] ), "-", target_type, "/Baseline/")
#paste0(auxPath, "/TB_SIM_OUT/", ctry, "/Vaccines/Vac_", sprintf("T_%d-E_%.2f-C_%.2f-W_%.2f", as.integer(aux_info["age"]), aux_info["efficacy"], aux_info["coverage"], aux_info["waning"] ), "/", line_tran, "/Baseline/")
            } else{
                pathout_vaccine <- paste0(auxPath, "/TB_SIM_OUT/", ctry, sprintf("_cm_%d", mode_contact), "/Vaccines/", line_tran, "/",
                                sprintf("T_%d-E_%.2f-C_%.2f-W_%.2f", as.integer(aux_info["age"]), aux_info["efficacy"], aux_info["coverage"], aux_Harris_waning[1] ), "-", target_type, "/Iters/")
#paste0(auxPath, "/TB_SIM_OUT/", ctry, "/Vaccines/Vac_", sprintf("T_%d-E_%.2f-C_%.2f-W_%.2f", as.integer(aux_info["age"]), aux_info["efficacy"], aux_info["coverage"], aux_info["waning"] ), "/", line_tran, "/Iters/")
            }
            ...
            if(iteration==0){
                savepath1 <- paste0(pathout_vaccine, "Output_vaccine.txt")
                ...
                savepath1 <- paste0(pathout_vaccine, "Output_vaccine_It_", sprintf("%03d", iteration), ".txt")
                write.table(out_df, savepath1, col.names=T, row.names=F, quote=F)
...
get_vaccine_info <- function(path_to_profile){
    ...
    data_pop <- read.table(path_to_profile, header=F, nrow=2)
    Harris_waning <- as.numeric(data_pop$V3[2]) 
    ret <- list(transitions, target_type, Harris_waning, values)
    return(ret)
}
...
get_inc_mort_fromObject <- function(obj1, obj2){
    ...
    df1 <- data.frame(t = 1999 + seq(1:(length(imp))), popu = pop, inc_annual = i, inc_acum = imp, mort_annual = m, mort_acum = mort,
                      inc_vac_annual = i_vac, inc_vac_acum = imp_vac, mort_vac_annual = m_vac, mort_vac_acum = mort_vac)    
    return(df1)
}
