...
int deriv(int pfoi, double gp[global_parameters][AG][EG],double vp[variable_parameters][AG][EG],double rp[regional_parameters][AG][EG],double cm[AG][AG][EG][EG], double s[states][AG][EG], double ds[states][AG][EG], double f_micro[AG][EG][internal_fluxes], double t,double S[states]);
int calculate_fluxes(int ppp, int a, int e,double t,double gp[global_parameters][AG][EG],double vp[variable_parameters][AG][EG],double rp[regional_parameters][AG][EG],double cm[AG][AG][EG][EG], double s[states][AG][EG],double f[fluxes],double S[states]);

double cdf(int a, int av, double wa);
...
void R2C_runner(char **mf, int *vaciter, int *con_mod, int *vaccinate, double *objeto1, double *objeto2, int *out_handler){
    ...
    master_file_reader(master_file, cm_mode_all);
    ...
    //load_cm_modified(cm_global, cm);
    load_contact_matrix(cm_global, cm);
	read_pyr_coef();

    if (strcmp(name_country, simulation_name) != 0)
        sprintf(aux_str, "../Outputs/%s_cm_%d/Iter/%s/Fitted_inputs/Parameters_to_fit.txt", name_country, cm_mode_all, simulation_name);
    else
	    sprintf(aux_str, "../Outputs/%s_cm_%d/Fitted_inputs/Parameters_to_fit.txt", simulation_name, cm_mode_all);

    double aux_harris[5][AG];
    load_fitting_parameters(aux_str, gp, rp, aux_harris);
    ...
    for(a=0; a<AG; a++){
        for(e=0; e<EG; e++){
	        rp[3][a][e] = rp_base[3][a][e]*aux_harris[0][a]; //Initial diagnosis rate smear-pos
			for(i=4; i<6; i++)
				rp[i][a][e] = rp_base[i][a][e]*aux_harris[0][a]*rp[18][a][e];//Diagnosis rate smear-neg. and extra-pulm, times eta
			rp[6][a][e] = aux_harris[1][a];
            rp[7][a][e] = aux_harris[2][a];
			rp[8][a][e] = aux_harris[3][a];
            rp[2][a][e] = aux_harris[4][a];
        }
	}
    ...
    if(*vaccinate == 0){
        ...
        /*if(*vaciter==0){
            ...
            sprintf(VacProfile, "../Inputs_Iter/%s/Perfil/perfil_%03d.txt", name_country, *vaciter);
        }*/
        sprintf(VacProfile, "../Inputs/General_Inputs/perfil.txt");
...
int calculate_fluxes(int ppp, int a, int e, double t,double gp[global_parameters][AG][EG],double vp[variable_parameters][AG][EG],double rp[regional_parameters][AG][EG],double cm[AG][AG][EG][EG], double s[states][AG][EG],double f[fluxes],double S[states])
{
	...
	for(i=0;i<6;i++){
        ...
        f[29+i] = vp[6][a][e]*rp[9][a][e]*vp[11][a][e]*gp[5][a][e]*lambda*s[9+i][a][e];    //After treatment without E_p effect
    }
	//f[35] = vp[6][a][e]*vp[10][a][e]*rp[9][a][e]*vp[11][a][e]*gp[5][a][e]*lambda*s[2][a][e]; //Before treatment (from slow to fast latency)
    f[35] = vp[6][a][e]*rp[9][a][e]*vp[11][a][e]*gp[5][a][e]*lambda*s[2][a][e]; //Before treatment (from slow to fast latency) without E_p effect
	...
	for(i=0;i<3;i++){
        ...
        f[64+i] = vp[6][a][e]*rp[9][a][e]*vp[11][a][e]*gp[5][a][e]*lambda*s[15+i][a][e]; //Without E_p
        //f[64+i] = 0.0;
    }
...
int deriv(int pfoi, double gp[global_parameters][AG][EG],double vp[variable_parameters][AG][EG],double rp[regional_parameters][AG][EG],double cm[AG][AG][EG][EG], double s[states][AG][EG], double ds[states][AG][EG], double f_micro[AG][EG][internal_fluxes], double t,double S[states])
{
	...
	for(a=0;a<AG;a++)
	{
		for(e=0;e<EG;e++)
		{
			...
			err=calculate_fluxes(pfoi, a, e, t, gp, vp, rp, cm, s, f, S);
...
int rk4(double gp[global_parameters][AG][EG],double vp[variable_parameters][AG][EG],double rp[regional_parameters][AG][EG],double cm[AG][AG][EG][EG], double s[states][AG][EG], double S[states], double f_micro[AG][EG][internal_fluxes], double t)
{
    ...
    //1 para apagar la escritura de foi, 0 para guardarla
	err=deriv(1, gp, vp, rp, cm, s, ds, fs, t, S);                /*ds=f(x0)*/
	if(err==1)
	{
	...
	err=deriv(1, gp, vp, rp, cm, st, dst, f_m1, t+hh, St);           /*st es x1, dst=f(x1)*/
	if(err==1)
		return err;
	...
	err=deriv(1, gp, vp, rp, cm, st, dsm, f_m2, t+hh, St);           /*xt es x2, dxm=f(x2)*/
	if(err==1)
		return err;
	...
	err=deriv(1, gp, vp, rp, cm, st, dst, f_m3, t+H, St);           /*dst=f(x3)*/
	if(err==1)
		return err;
...
void ageing(double ds[states][AG][EG], double s[states][AG][EG],double S[states],double vp[variable_parameters][AG][EG],double t){
	int i, a, e;
	double sick_density, numerator, denominator, vac_flux[reservories+1][EG-1], vac_flux_aux[reservories+1][EG-1], flux, flux_aux;
    double vac_flux_waning[reservories+1][EG-1];
	...
	for(a=1; a<AG; a++){
		for(e=0; e<EG-1; e++){
            if(mass_camp==1){
                vac_flux_aux[0][e] = vp[1][a][e]*s[0][a][e]/(double)time_camp;
			    vac_flux_aux[1][e] = vp[2][a][e]*s[1][a][e]/(double)time_camp;
			    vac_flux_aux[2][e] = vp[2][a][e]*s[2][a][e]/(double)time_camp;
            }
            else{
                vac_flux_aux[0][e] = 0.0;
			    vac_flux_aux[1][e] = 0.0;
			    vac_flux_aux[2][e] = 0.0;
            }
            vac_flux[0][e] = vp[1][a][e]*s[0][a-1][e];
			vac_flux[1][e] = vp[2][a][e]*s[1][a-1][e];
			vac_flux[2][e] = vp[2][a][e]*s[2][a-1][e];

            vac_flux_waning[0][e] = cdf(a, target, waning_harris)*s[0][a][e];
            vac_flux_waning[1][e] = cdf(a, target, waning_harris)*s[1][a][e];
            vac_flux_waning[2][e] = cdf(a, target, waning_harris)*s[2][a][e];

			for(i=3; i<reservories; i++){
				vac_flux[i][e] = 0.0;
                vac_flux_aux[i][e] = 0.0;
                vac_flux_waning[i][e] = 0.0;
            }
			for(i=9; i<18; i++){
                vac_flux[i][e] = vp[28][a][e]*s[i][a-1][e];
                vac_flux_waning[i][e] = cdf(a, target, waning_harris)*s[i][a][e];
                if(mass_camp==1){
                    vac_flux_aux[i][e] = vp[28][a][e]*s[i][a][e]/(double)time_camp;
                }
                else{
				    vac_flux_aux[i][e] = 0.0;
                }
            ...
            vac_flux_aux[reservories][e] = 0.0;
            vac_flux_waning[reservories][e] = 0.0;
			for(i=0; i<reservories; i++){
		...
		for(i=0; i<=reservories; i++){
            ds[i][a][0] += (s[i][a-1][0] - s[i][a][0] - vac_flux[i][0])/(double)age_window - vac_flux_aux[i][0] + vac_flux_waning[i][1]/(double)age_window;
            ds[i][a][1] += (s[i][a-1][1] - s[i][a][1] + vac_flux[i][0])/(double)age_window + vac_flux_aux[i][0] - vac_flux_waning[i][1]/(double)age_window;
